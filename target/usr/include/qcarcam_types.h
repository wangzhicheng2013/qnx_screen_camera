...
#define QCARCAM_MAX_REG_BLOCK 64
...
typedef enum
{
    ...
    QCARCAM_INPUT_TYPE_USER_MONITOR  = 12,  ////< USER monitor
    QCARCAM_INPUT_TYPE_DRIVER_RECORDER  = 13,  ///<driver recorder
    QCARCAM_INPUT_TYPE_TOF_DEPTH         =14,   ///Comprehensive analysis of depth camera
...
typedef enum
{
    ...
    CUSTOM_PARAM_GET_CAMERA_TYPE = 0x800001,/// < Get camera type
    CUSTOM_PARAM_GET_DID_INFO = 0x800002,/// < Get camera did infomation
    CUSTOM_PARAM_GET_DESER_INFO = 0x800003,/// < Get deserializer infomation
    CUSTOM_PARAM_DESER_GPIO_TEST = 0x800004, /// < Start deserializer gpio test
    CUSTOM_PARAM_SET_TEST_PARRENT_MODE = 0x800005, /// < Set camera channel to test mode
...
/// @brief structure to hold remote write/read parameters
typedef struct
{
    int write;
    unsigned int reg_addr;
    unsigned int reg_val[QCARCAM_MAX_REG_BLOCK];
    unsigned int length;
}qcarcam_remotecci_config_t;
...
typedef union
{
    ...
    qcarcam_remotecci_config_t remotecci_config;     ///< Remote read/write settings
    ...
    qcarcam_param_req_info_t request;
